...
def getType(hand, part2=False):
    # keep track of jokers -- ignore for part 1
    numjokers=0

    if part2:   # if part 2, get 
...
def solve(isPart2=False):
    ...
    # sort cards based primarily on strength, followed by on 
    sortedgame = sorted(game, key=lambda hand:(15**10)*hand[0] + sum([15**(6-index)*faceValue for index,faceValue in enumerate(hand[1])]))
